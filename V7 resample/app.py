...
def auto_generate_strategy():
    """
    Auto-generate strategy with random entry conditions and TP/SL
    """
    try:
        ...
        print(f"🤖 Auto-generating strategy:")
        print(f"   Long signals: {long_signals}, Short signals: {short_signals}")
        print(f"   Indicators per signal: {indicators_per_signal}")
        print(f"   Profit levels: {profit_levels}, Step: {profit_step}")
        ...
        print(f"   Long entries: {len(modified_strategy['entry_conditions']['long'])}")
        print(f"   Short entries: {len(modified_strategy['entry_conditions']['short'])}")
        ...
        print(f"❌ Error auto-generating strategy: {e}")
        import traceback
        traceback.print_exc()
...
@app.route('/api/strategies/<filename>', methods=['GET', 'DELETE'])
def handle_strategy(filename):
    """
    GET: Load a strategy
