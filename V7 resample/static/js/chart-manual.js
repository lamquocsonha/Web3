...
class ChartManual {
    constructor(containerId) {
        console.log('=== ChartManual constructor called ===');
    ...
    init() {
        console.log('🔵🔵🔵 ChartManual.init() called - START ===');
        ...
        // Create chart
        console.log('📊 Creating chart with createChart()...');
        ...
        // Initialize indicators modal with chartType
        this.indicatorsModal = new ChartIndicatorsModal(this.chart, 'manual');
        ...
        setTimeout(() => {
            console.log('📊 ChartManual: Loading chart data...');
            this.loadChartData();
        }, 1000);
    }
    
    loadUploadedData(chartData, volumeData, metadata) {
        console.log('=== loadUploadedData called ===');
        console.log('Chart data length:', chartData ? chartData.length : 0);
        console.log('Volume data length:', volumeData ? volumeData.length : 0);
        console.log('Metadata:', metadata);
        
        if (!chartData || chartData.length === 0) {
            console.warn('No chart data to load');
            return;
        }
        
        console.log('Loading uploaded data to manual chart:', chartData.length, 'bars');
    ...
    async loadOnlineData(connection) {
        ...
        console.log('📡 Loading online data for:', profile.exchange, '-', symbol);
        ...
        try {
            ...
            // Simulate fetching data from exchange
            // TODO: Replace with real API call to exchange
            ...
            console.log('✅ Received', response.data.length, 'candles from exchange');
            ...
            alert('❌ Lỗi khi tải dữ liệu online: ' + error.message);
    ...
    async fetchExchangeData(profile, symbol) {
        ...
        // TODO: Implement real API call based on exchange type
    ...
    async fetchLatestCandle(profile, symbol) {
        ...
        // For now, simulate latest candle
        const now = Math.floor(Date.now() / 1000);
        const basePrice = 1350;
        const volatility = 20;
    ...
    setupUTCSelector() {
        ...
        if (utcSelector) {
            // Load saved UTC offset
            const savedUtc = localStorage.getItem('chartUTC_manual');
            if (savedUtc !== null) {
                this.utcOffset = parseInt(savedUtc);
                utcSelector.value = savedUtc;
                console.log('Loaded saved UTC:', this.utcOffset);
            }
            
            utcSelector.addEventListener('change', (e) => {
                this.utcOffset = parseInt(e.target.value);
                console.log('UTC changed to:', this.utcOffset);
                
                // Save to localStorage
                localStorage.setItem('chartUTC_manual', this.utcOffset);
                
                // Update clock display
                this.updateClockDisplay();
                
                // Update chart timeFormatter
    ...
    async loadChartData() {
        try {
            ...
            if (window.UploadDataManager && window.UploadDataManager.csvData && window.UploadDataManager.csvData.length > 0) {
                ...
                // STEP 2: Data already resampled by backend
                const resampledBars = rawBars; // Backend handled resample
    ...
    // Resample removed - backend handles this via /api/resample-data
