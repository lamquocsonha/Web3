...
class ChartStrategy {
    ...
    init() {
        ...
        setTimeout(() => {
            console.log('📊 ChartStrategy: Loading chart data...');
            this.loadChartData();
        }, 1000);
    }
    ...
    async loadChartData() {
        try {
            ...
            // No uploaded data available - show empty chart
            console.log('ℹ️ No uploaded CSV data available. Please upload a CSV file.');
            this.candlestickSeries.setData([]);
            if (this.volumeSeries) {
                this.volumeSeries.setData([]);
            }
...
// Note: Chart initialization is handled by strategy.js
// Do not initialize here to avoid duplicate charts
