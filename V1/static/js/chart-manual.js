...
class ChartManual {
    ...
    checkAndLoadUploadedData() {
        console.log('=== checkAndLoadUploadedData called ===');
        // Check if UploadDataManager has data
        setTimeout(() => {
            console.log('Checking UploadDataManager...');
            if (window.UploadDataManager && window.UploadDataManager.csvData) {
                console.log('Found uploaded data, loading to chart...');
    ...
    async loadChartData() {
        try {
            // If connected, data comes from WebSocket
            if (this.connectionManager.isConnected) {
